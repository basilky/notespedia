import $ from 'jquery'
import Vue from 'vue'
import App from './components/VerifyQp.vue'
...
var notes = new Vue({
  ...
  template: `
          ...
          <div v-else class="col-md-12 remove text-center" style="padding:30px;"><br><h2>No files are available for verification.</h2>
          </div>
          </div>`
});
$(document).ready(function ()
{
  ...
  fetch(`/backend/unverifiedfiles.php?type=3`)
    .then((resp) => resp.json())
    .then(function (Notes)
    {
      ...
      fetch(`/subjects.json?nocache=` + (new Date()).getTime())
        ...
        {
          fetch(`/backend/unverifiedfiles.php?type=4`)
            .then((resp) => resp.json())
